...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      // Allow NIMA domains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*nutribuddy\.com$/)) {
        return true;
      }
      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="visual-edit" -c user.email="support@nima.com" add "${targetFile}"`);
            execSync(`git -c user.name="visual-edit" -c user.email="support@nima.com" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
