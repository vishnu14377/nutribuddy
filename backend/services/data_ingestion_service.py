            ...
            for i in range(20):
                ...
                if pd.notna(title) and item_uuid not in seen_uuids:
                    ...
                    item = {
                        ...
                        'image_url': str(row.get(f'{prefix}imageUrl', '')) if pd.notna(row.get(f'{prefix}imageUrl')) else '',
                        'restaurant_name': restaurant_name,
                        'delivery_time': '20-35 min',
                        'rating': rating,
                        'price': round(price, 2),
                        'uber_uuid': item_uuid,
                        'tags': str(row.get(f'{prefix}tags/0', '')) if pd.notna(row.get(f'{prefix}tags/0')) else ''
                    }
